...
class KitchenOrder(models.Model):
    ...
    _description = "Create Kitchen Orders"
    state = fields.Selection([('in_process','En Proceso'),('finished','Finalizado')])
    # voter_ids = fields.One2many('situational_room.voter','voter_id',string="Votantes Movilizados")

    @api.model
    def create_pos_kitchen_order(self, order, orderLine, user, client):
        result = self.create({'state':'in_process'})
        return result

# class KitchenLines(models.Model):
#     _name = "pos.kitchen.line"
#     _description = "Create Kitchen Lines for Kitchen Orders"
